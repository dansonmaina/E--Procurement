...
class Xcrud
{
	...
	protected $confirm_delete_action =false;
	protected $confirm_delete_action_msg ="Do you wish to proceed";
	protected $confirm_edit_action =false;
	protected $confirm_edit_action_msg ="Do you wish to proceed";
	protected $confirm_add_action =false;
	protected $confirm_add_action_msg ="Do you wish to proceed";
	...
	public function confirm_edit_action_msg($msg)
    {
        $this->confirm_edit_action_msg = $msg;
        return $this;
    }
	
	public function confirm_edit_action($status)
    {
        $this->confirm_edit_action = $status;
        return $this;
    }
	
	public function confirm_add_action($status)
    {
        $this->confirm_add_action = $status;
        return $this;
    }
	
	public function confirm_add_action_msg($msg)
    {
        $this->confirm_add_action_msg = $msg;
        return $this;
    }
    ...
    protected function _insert($postdata, $no_processing = false, $no_processing_fields = array())
    {
		...
		if (!$this->demo_mode && $this->is_log == true){
			$q_log = 'INSERT INTO `' . $this->table . '` (' . implode(',', array_keys($set[$this->table])) . ') VALUES (' .
                implode(',', $set[$this->table]) . ')';	
			$db->query('INSERT INTO `logs` (record_id,action,table_name,data,updated_by) VALUES ("' . $ins_id . '","INSERT","' . $this->table . '","' . $q_log . '" , "' . $user_id . '")');
		}	
    ...
    protected function _update($postdata, $primary)
    {
		...
		if (!$this->demo_mode && $this->is_log == true){
			 $q_log = "UPDATE `{$this->table}` AS `{$this->table}` " . implode(' ', $joins_qr) . " SET " . implode(",\r\n", $set) .
                    " WHERE `{$this->table}`.`{$this->primary_key}` = " . $db->escape($primary);	
			$db->query('INSERT INTO `logs` (record_id,action,table_name,data,old_record,updated_by) VALUES (' . $db->escape($primary) . ',"UPDATE","' . $this->table . '","' . $q_log . '","' . $row_log_json . '","' . $user_id . '")');
		}	
            ...
            if (!$this->join)
            {
				...
				if (!$this->demo_mode && $this->is_log == true){
					$q_log = "DELETE FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($this->primary_val) .
                        " LIMIT 1";	
					$db->query('INSERT INTO `logs` (record_id,action,table_name,data,old_record,updated_by) VALUES (' . $db->escape($this->primary_val) . ',"DELETE","' . $this->table . '","' . $q_log . '","' . $row_log_json . '",$user_id)');
			    }			
            }
            else
            {
    ...
    protected function params2save()
    {
        return array(
            ...
            'confirm_delete_action_msg',
            'confirm_edit_action',
            'confirm_edit_action_msg',
            'confirm_add_action',
            'confirm_add_action_msg',
            'language',
            'field_params',
            'mass_alert_create',
            'mass_alert_edit',
    ...
    protected function _render_list_buttons(&$row)
    {
        ...
        {      	
			...
			if ($this->confirm_edit_action)
            {
                $tag['data-confirm'] = $this->confirm_edit_action_msg;
            }
        ...
        if (!isset($this->hide_button['remove']) && !$this->table_ro && $this->is_remove($row))
        {
            ...
            if ($this->confirm_delete_action)
            {
                $tag['data-confirm'] = $this->confirm_delete_action_msg;
            }
    ...
    public static function load_js()
    {
        ...
        if (Xcrud_config::$load_jquery)
		    ...
		    $out .= '<script src="Admin/Vertical/dist/assets/js/vendor.min.js"></script>';
    ...
    protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = '')
    {
        ...
        if (!isset($this->hide_button[$name]))
        {
			...
			if($task == "create"){
				if($this->confirm_add_action == true){
					$tag['data-confirm'] = $this->confirm_add_action_msg;
				}					
			}
